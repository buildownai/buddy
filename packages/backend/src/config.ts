...
const ConfigSchema = z.object({
  ...
  surrealdb: z
    .object({
      url: z.string().url().default('ws://127.0.0.1:8000').describe('SurrealDB connection URL'),
  ...
  llm: z
    .object({
      url: z.string().url().default('http://127.0.0.1:11434/v1').describe('LLM API URL'),
      apiKey: z.string().default('API_KEY').describe('The LLM Provider API Key'),
      ...
      models: z.object({
        ...
        small: z.string().default('qwen2.5-coder:latest').describe('Default LLM model to use'),

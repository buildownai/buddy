  ...
  return streamSSE(
    ...
    async (stream) => {
      ...
      try {
        ...
        await chatHistoryRepository.updateConversationSummary(conversationId, summary)
      } catch (err) {
        logger.error({ err, projectId, conversationId }, 'Error during chat response generation')
        await sendSSE({
          event: 'error',

...
const paramSchema = z.object({
  ...
  version: z.string().optional().default('latest').describe('The package version'),
})
...
export const getNpmPackageInfo: ToolEntry = {
  fn: async (input: unknown) => {
    try {
      const { name, version } = validateParams(paramSchema, input)

      const url = new URL(`${name}`, 'https://registry.npmjs.org/')
      const response = await fetch(url)

      if (!response.ok) {
        logger.error({ url, status: response.status }, response.statusText ?? 'Failed to fetch')
  ...
  schema: {
    ...
    function: {
      ...
      description: 'Get npm package information for packages other than the current project',

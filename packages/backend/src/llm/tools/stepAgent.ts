...
const tools = Array.from(availableFunctions.entries()).map(
  ([name, { schema }]) => schema
);
...
const paramSchema = z.object({
  steps: z.array(
    z
      .string()
      .describe("The single task to perform in this step in natural language")
  ),
});
...
Try to categorize the output in success or fail.
      ...
      for (const step of steps) {
        ...
        if (response.message.tool_calls) {
          ...
          await sendSSE(
            "info",
            `Step: step => response: ${response.message.content}`
          );
        }
      }
  ...
  schema: {
    ...
    function: {
      name: "step_agent",
      description:
        "A list of steps in natural language which are executed sequentially by an llm",
